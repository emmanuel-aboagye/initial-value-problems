...
def mov_bed_reactor_model(x, W):
    # parameters
    k = 0.1  # mol/(kg-cat atm)
    Ka = 1.5  # 1/atm
    kd = 0.75  # 1/s-atm
    Fao = 10 # mol/s
    Pao = 20 # atm
...
mbr_initial_cond = [0., 0., 10]
mbr_W_sim = np.linspace(0, 500., 10000)
solution8 = odeint(mov_bed_reactor_model, mbr_initial_cond, mbr_W_sim)
...
plt.figure(dpi=1000)
plt.plot(m.time,x.value,'bo',label='Predicted (k='+str(np.round(k,2))+')')
